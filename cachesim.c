...
struct cache_t {
  int num_blocks;		// # sets
  int words_per_block;		// block size
  int assoc;		// associativity

  enum cache_policy replacement;       // cache replacement replacement
...
void setup_caches()
{
	...
	C = (struct cache_t *)calloc(1, sizeof(struct cache_t));

	C->num_blocks = icache_info.num_blocks;
	C->words_per_block = icache_info.words_per_block;
	C->assoc = icache_info.associativity;
	C->replacement = icache_info.replacement;


	C->blocks= (struct cache_blk_t **)calloc(C->num_blocks, sizeof(struct cache_blk_t));

	int i;
	for(i = 0; i < C->num_blocks; i++)
	{
		C->blocks[i] = (struct cache_blk_t *)calloc(C->assoc, sizeof(struct cache_blk_t));
	}
...
void handle_access(AccessType type, addr_t address)
{
	...
	index = (address/C->words_per_block)%C->num_blocks;
	tag = (address/C->words_per_block)/C->num_blocks;

	for(i = 0; i < C->assoc; i++)
	{
		if(C->blocks[index][i].tag == tag && C->blocks[index][i].valid == 1)
		{
			C->blocks[index][i].last = now;
			hit = 1;
			break;
		}
		else if(C->blocks[index][i].valid == 0)
		{
			space = i;
			break;
		}
	...
	if(hit != 1)
	{
		if(space != -1)
		{
			C->blocks[index][space].tag = tag;
			C->blocks[index][space].valid = 1;
			C->blocks[index][space].last = now;
			C->blocks[index][space].first = now;
			if(type == Access_I_FETCH)
			{
				C->blocks[index][space].dirty = 0;
			}
		}
		else if(C->replacement == 0)
		{
			int min = C->blocks[index][0].last;
			int minindex = 0;

			for(i = 1; i < C->assoc; i++)
			{
				if(C->blocks[index][i].last < min)
				{
					min = C->blocks[index][i].last;
					minindex = i;
				}
			}
		}
